...
class FNN(object):
    ...
    def train(self, input, output, learning_rate=1e-3, batch_size=10,
        ...
        Args:
            input(numpy.ndarray): Input NumPy ndarray used for training model.
            output(numpy.ndarray): Output regression or labels from NumPy ndarray used for training model.
        ...
        if input.shape[1] != self._in_size:
            raise ValueError('Input doesn\'t match model input size.')
        if output.shape[1] != self._out_size:
        ...
        # Begin Training
        # ================================================
        total_batches = int(np.ceil(input.shape[0] * 1. / batch_size))

        for epoch in range(n_epochs):
            # --------------------------------
            # Randomize Inputs and Outputs
            # --------------------------------
            perm = np.random.permutation(np.arange(input.shape[0]))
            input = input[perm]
            output = output[perm]
            ...
            if epoch_print and epoch % epoch_print == 0:
                train_accuracy = self._sess.run(accuracy, feed_dict={self._x: input,
                                                                     self._y: output})
                if self._tensor_board:
                    s = self._sess.run(merged_summary, feed_dict={self._x: input,
                                                                  self._y: output})
                    writer.add_summary(s, batch_iter + (total_batches * epoch))
                print('epoch %d, training accuracy %g' % (epoch, train_accuracy))

            while curr_batch < input.shape[0]:

                # Get Data Batch
                if (curr_batch + batch_size) < input.shape[0]:
                    batch_x = input[curr_batch:curr_batch + batch_size]
                    batch_y = output[curr_batch:curr_batch + batch_size]
                    curr_batch += batch_size
                else:
                    batch_x = input[curr_batch:]
                    batch_y = output[curr_batch:]
